...
class LinearUCBLearner():
    ...
    def compute_ucbs(self):
        ...
        for arm in self.arms:
            ...
            ucb = np.dot(self.theta.T, arm) + \
                  self.c * np.sqrt(np.dot(arm.T, np.dot(np.linalg.inv(self.M), arm)))
            ucbs.append(ucb[0][0])
    ...
    def update_estimations(self, arm_idx, reward):
    ...
    def update(self, arm_idx, reward):
        ...
        self.update_estimations(arm_idx, reward)
